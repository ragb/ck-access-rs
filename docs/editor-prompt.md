...
- **Persisting (Store):** a bulk dump — even a slot-addressed one — is **lost on a power cycle**. To make a save stick (the panel STORE button / Melas's "Store"), mirror Melas: dump the whole Live Set to the **edit buffer** (`bulkHeaderCurrentBuffer()` … blocks … `bulkFooterCurrentBuffer()`), then send `storeToFlash(device, page, sound)` naming the destination slot (1-based `1..=20` / `1..=8`). "Store to…" = the chosen slot; "Store" = the slot the Live Set was loaded from. Without this frame the edit reverts on power-off.
